...
test('vendor payment summary remains empty until an invoice calculation is selected',()=>{
 ...
 ],combinedVendorInvoices:[],activeInvoiceId:'',el:id=>id==='vendorPaymentSummary'?host:null,esc:String,money:n=>'₹'+n,yuan:n=>'¥'+n};
...
test('mixed-vendor combined invoice replaces child payable rows but preserves child IDs',()=>{
 ...
 combinedInvoiceDetail:()=>'<div>PO-1 + PO-2</div>',el:id=>id==='vendorPaymentSummary'?host:null,esc:String,money:n=>'₹'+n,yuan:n=>'¥'+n};
 ...
 assert.match(host.innerHTML,/Finalized · Pending/);
...
test('finalized LG bill persists in payment summary after reload without active selection',()=>{
 const start=html.indexOf('    var selectedPaymentBills='),end=html.indexOf('    function renderPurchaseHistory(meta)',start);
 const host={innerHTML:'',querySelectorAll:()=>[]};
 const context={me:{isAdmin:true,role:'owner',roles:['owner']},purchaseHistory:[{id:'PO-1',vendor:'Vendor A',billNo:'SBGB174',datePurchase:'2026-09-02',paymentSummary:{amount:90000,paidAmount:0,balanceDue:90000}}],activeInvoiceId:'',combinedVendorInvoices:[{id:'CVI-1',vendor:'Vendor A',poIds:['PO-1'],lgDate:'2026-09-18',lgBillNumber:'LG-001',childBills:{'PO-1':{billValueYuan:5840}},finalized:{basis:'vendor',amountInr:90000}}],combinedInvoiceDetail:()=>'<div>calculation</div>',el:id=>id==='vendorPaymentSummary'?host:null,esc:String,money:n=>'₹'+n,yuan:n=>'¥'+n};
 vm.runInNewContext(html.slice(start,end)+'\nthis.render=renderVendorPaymentSummary;',context);context.render();
 assert.match(host.innerHTML,/LG-001/);assert.match(host.innerHTML,/Vendor bills ¥5840/);assert.match(host.innerHTML,/data-combined-detail=\"CVI-1\"/);
});
