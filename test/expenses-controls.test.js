...
test('one ledger movement cannot be selected twice in the same reconciliation',()=>{
  ...
  stored.bankReconciliationDrafts[id]={id,account,nature:'SANKI',transactions:[{date,description:'Combined payment',reference:'BANK-1200',debit:1200,credit:0,balance:-1200},{date,description:'Unrelated debit',reference:'BANK-600',debit:600,credit:0,balance:-1800}],summary:{from:date,to:date,openingBalance:0,closingBalance:-1800,totalDebits:1800,totalCredits:0,validated:true},resolutions:{},matchingPolicy:'strict_identity_v2',temporaryFile:'',createdAt:new Date().toISOString(),createdBy:'prashant',expiresAt:'2099-01-01T00:00:00.000Z'};fs.writeFileSync(expenseFile,JSON.stringify(stored));
