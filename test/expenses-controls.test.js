...
test('strict reconciliation policy separates same-date same-amount parties and preserves manual decisions',()=>{
  const account='Prashant Axis 3645',store={expenses:{'EX-00032':{id:'EX-00032',nature:'SANKI',status:'paid',approvedAt:'2026-08-24T10:00:00Z',vendor:'Kalu Flowers & Fruits',amount:200,paidAmount:200,payments:[{id:'PAY-001',date:'2026-08-24',amount:200,account}]},'EX-00120':{id:'EX-00120',nature:'SANKI',status:'paid',approvedAt:'2026-08-27T10:00:00Z',vendor:'Vi',amount:651.90,paidAmount:651.90,payments:[{id:'PAY-001',date:'2026-08-27',amount:651.90,account}]}},adjustments:[],vendorAdvances:[],receipts:[],transfers:[],receivables:{},vendorOpeningPayables:[],paytmSettlements:[],oneTimeMigrations:{},auditLog:[],auditSeq:0,bankDateOverrides:{'EX-00120/PAY-001':{bankDate:'2026-08-27',bankReference:'660545756771'}},bankReconciliationDrafts:{D1:{id:'D1',account,nature:'SANKI',transactions:[{date:'2026-08-24',description:'UPI/P2M/623647818325/Ferns n petals',reference:'623647818325',debit:200,credit:0},{date:'2026-08-24',description:'Manual row',reference:'KEEP',debit:50,credit:0},{date:'2026-08-27',description:'UPI/P2M/660545756771/Euronet Services India',reference:'660545756771',debit:651.90,credit:0}],summary:{from:'2026-08-24',to:'2026-08-27',closingBalance:-901.90},resolutions:{'bank-1':{action:'exclude',reason:'Keep manual decision'}}}}};
  assert.equal(applyStrictReconciliationIdentityPolicy(store),true);assert.equal(store.bankReconciliationDrafts.D1.matchingPolicy,'strict_identity_v3');assert.deepEqual(store.bankReconciliationDrafts.D1.resolutions['bank-1'],{action:'exclude',reason:'Keep manual decision'});
  const report=store.oneTimeMigrations['require-date-amount-direction-and-party-identity-for-bank-auto-match-v3'].updatedDrafts[0];assert.deepEqual(report.removedAutomaticMatches.map(x=>[x.rowId,x.appId]),[['bank-0','EX-00032/PAY-001']]);assert.equal(report.preservedResolutionCount,1);assert.equal(report.removedAutomaticMatches.some(x=>x.appId==='EX-00120/PAY-001'),false,'a previously confirmed bank reference stays matched');assert.equal(store.auditLog.at(-1).action,'BANK_AUTO_MATCH_POLICY_UPDATED');assert.equal(applyStrictReconciliationIdentityPolicy(store),false);
});
