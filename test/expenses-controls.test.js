...
test('3645 carries excluded bank truth into later statement periods and keeps one salary-advance bank match',()=>{
  ...
  try{
    ...
    const current=invoke('POST','/api/expenses/bank-statements/reconcile',{role:'admin',body:{draftId:'current',account}}).body;assert.equal(current.actualBankClosing,50);assert.equal(current.excludedBankNet,-80);assert.equal(current.bankClosing,130);
...
test('salary advance funding links one internal transfer without double-counting the source debit',()=>{
  ...
  assert.equal(source.entries.filter(x=>x.id===made.body.transfer.id&&x.debit===10000).length,1);assert.equal(source.entries.some(x=>x.id==='ADV-00030'),false,'linked advance must not create a second bank debit');assert.equal(dest.entries.filter(x=>x.id===made.body.transfer.id&&x.credit===10000).length,1);
