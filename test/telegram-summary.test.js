...
test('daily Telegram summary separates activity, pending work and employee settlement',()=>{
  ...
  assert.equal(out.recorded.total,763);assert.equal(out.recorded.business,663);assert.equal(out.recorded.personal,100);assert.equal(out.totalPayments,2473);assert.equal(out.approvedPending[0].amount,190);assert.equal(out.awaitingApproval[0].amount,100);assert.equal(out.settlements.find(x=>x.employee==='Prashant').balance,-4045.08);
  const text=telegram.formatAccountingSummary(out,'5 September 2026');
  assert.match(text,/The Printing Solutions ₹2,000 — Prashant Axis 3645/);assert.match(text,/Company owes Prashant ₹4,045\.08/);assert.match(text,/Action:<\/b> pay 1 expense ₹190 · review 1 ₹100/);assert.doesNotMatch(text,/proof|bill photo/i);
});
