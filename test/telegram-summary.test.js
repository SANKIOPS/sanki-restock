...
test('daily Telegram summary separates activity, pending work and employee settlement',()=>{
  ...
  },transfers:[],adjustments:[],receipts:[],bankTruthMovements:[{id:'BT-1',nature:'SANKI',account:'Prashant Axis 3645',date:'2026-09-05',credit:955,debit:0,accountingExcluded:true}]};
  ...
  assert.deepEqual(out.employeeFunds,[{employee:'Prashant',account:'Prashant Axis 3645',nature:'SANKI',balance:-2094.08}]);
  const text=telegram.formatAccountingSummary(out,'5 September 2026');
  assert.match(text,/The Printing Solutions ₹2,000 — Prashant Axis 3645/);assert.match(text,/Company owes Prashant ₹2,094\.08 — Prashant Axis 3645/);assert.match(text,/Company owes Prashant ₹300/);assert.match(text,/EX-C · Claimed item · paid personally ₹500 − reimbursed ₹200/);assert.doesNotMatch(text,/Company owes Arshpreet|Company owes Pradeep|₹4,045\.08/);assert.match(text,/Action:<\/b> pay 1 expense ₹190 · review 1 ₹100/);assert.doesNotMatch(text,/proof|bill photo/i);
});
