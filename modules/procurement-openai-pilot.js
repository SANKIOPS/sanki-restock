...
function stylingPrompt(group,styling) {
  ...
  // Purchase labels can be wrong. In particular, an internal "muscle fit"
  // T-shirt may actually be a loose knit in the reference photograph.
  const selectedFit=style.fit;
  const fitRule=category!=='upper'
    ...
    :selectedFit==='Fitted'||selectedFit==='Slim fit'
      ? `Use a ${selectedFit.toLowerCase()} silhouette ONLY if the original garment visibly has that construction. If the reference has a loose, boxy or dropped-shoulder cut, retain that actual cut instead; do not tighten, shorten or change the original to satisfy this styling choice.`
      :selectedFit==='Oversized'||selectedFit==='Boxy / relaxed'
        ? `Use the selected ${selectedFit.toLowerCase()} cut ONLY if the original garment supports it; otherwise preserve the actual seams and proportions visible in the reference.`
        :selectedFit==='Normal fit'?'Use a regular fit only if the reference supports it; preserve any real dropped shoulders or loose cut.':'Match the garment silhouette and shoulder seams visible in the reference; do not infer a fit from purchase labels.';
