...
function imagePrompt(group, type, styling, hasContinuityReference=false) {
  ...
  const common = `The ORIGINAL PRODUCT PHOTO is the sole authority for the featured garment. First inspect it and reproduce the same garment category and construction: neckline/collar, full sleeve length, shoulder seams, silhouette, hem, colour, pattern and visible knit or surface texture. A long-sleeve V-neck knit must remain a long-sleeve V-neck knit, NEVER become a short-sleeve T-shirt; a loose or dropped-shoulder garment must not be made slim. Ignore any contradictory purchase title, product type or fit setting; do not guess unseen details. Change only the setting, pose and supporting outfit, never the featured garment. Do not invent a logo, fabric composition, unseen back, pockets or details. One garment, no collage, text or watermark.`;
