...
router.get('/api/expenses/account-ledger', (req, res) => {
  ...
  (s.vendorAdvances||[]).filter(x=>!x.accountingExcluded&&normalizedNature(x.nature)===nature&&x.account===account).forEach(x=>entries.push({id:x.id,date:x.date,kind:'vendor_advance',description:'Vendor advance · '+x.vendor+' · '+x.note,credit:0,debit:num(x.amount),proof:x.proof||'',reference:x.bankReference||x.id,by:x.createdBy||''}));
...
function appBankMovements(s,account,nature){const rows=[],n=normalizedNature(nature);
  ...
  (s.vendorAdvances||[]).filter(x=>!x.accountingExcluded&&x.account===account&&normalizedNature(x.nature)===n).forEach(x=>rows.push({id:x.id,date:x.date,createdAt:x.createdAt,description:'Vendor advance · '+x.vendor,reference:x.bankReference||'',proof:x.proof||'',debit:num(x.amount),credit:0}));
...
function reconciliationMatchPlan(bank,app,blockedBank,blockedApp,policy){
  ...
  candidates.forEach(x=>{if(x.exactAmount&&bankCounts.get(x.bi)===1&&appCounts.get(x.ai)===1)x.score+=20;});candidates.sort((a,b)=>b.score-a.score||a.days-b.days||a.bi-b.bi||a.ai-b.ai);
  const plan=new Map(),usedBank=new Set(),usedApp=new Set();candidates.forEach(x=>{if(usedBank.has(x.bi)||usedApp.has(x.ai))return;usedBank.add(x.bi);usedApp.add(x.ai);const unique=x.exactAmount&&bankCounts.get(x.bi)===1&&appCounts.get(x.ai)===1,confidence=x.referenceMatch?'exact':(x.identityMatch&&x.exactAmount?'strong':(unique?'strong':'possible'));plan.set(x.bi,Object.assign({},x,{confidenceLabel:confidence,requiresConfirmation:!(x.referenceMatch||(x.identityMatch&&x.exactAmount))}));});return plan;
}
function draftReconciliation(s,draft){
  ...
  const groupOrder={possible_match:0,amount_mismatch:0,matched:1,resolved:1,already_reconciled:1,missing_in_app:2,missing_in_bank:2},rowDate=x=>String((x.bank&&x.bank.date)||(x.app&&x.app.date)||''),rowTime=x=>String((x.bank&&x.bank.timestamp)||(x.app&&x.app.createdAt)||rowDate(x));rows.sort((x,y)=>{const gx=groupOrder[x.status]??9,gy=groupOrder[y.status]??9;if(gx!==gy)return gx-gy;if(gx===2)return rowDate(x).localeCompare(rowDate(y))||(x.status==='missing_in_app'?0:1)-(y.status==='missing_in_app'?0:1)||rowTime(x).localeCompare(rowTime(y))||String(x.id).localeCompare(String(y.id),undefined,{numeric:true});return Number(!!y.matchingExclusion)-Number(!!x.matchingExclusion)||rowTime(y).localeCompare(rowTime(x))||String(x.id).localeCompare(String(y.id),undefined,{numeric:true});});
