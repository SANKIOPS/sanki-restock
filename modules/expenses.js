...
router.get('/api/expenses/account-ledger', (req, res) => {
  ...
  if(nature==='SANKI'&&account===PAYTM_CLEARING_ACCOUNT)(s.paytmSettlements||[]).forEach(x=>entries.push({id:x.id,date:x.date,kind:'paytm_settlement',description:'Settled to '+x.bankAccount+(x.chargeAmount?' · charges ₹'+x.chargeAmount:''),credit:0,debit:num(x.grossAmount),settlement:x}));
...
function draftReconciliation(s,draft){
  const bank=draft.transactions||[],app=appBankMovements(s,draft.account).filter(x=>(!draft.summary.from||x.date>=draft.summary.from)&&(!draft.summary.to||x.date<=draft.summary.to)),used=new Set(),rows=[];
