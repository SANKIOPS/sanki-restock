...
function storeBankStatementImport(parsed,input){const b=input||{},s=loadStore(),account=String(b.account||''),nature=normalizedNature(b.nature);if(!ledgerAccountsForNature(s,nature).some(a=>a.toLowerCase()===account.toLowerCase()))return{success:false,error:'Select the correct account.'};if(!parsed.length)return{success:false,error:'No dated debit/credit transactions were found.'};s.bankStatements=s.bankStatements||{};const book=s.bankStatements[account]||(s.bankStatements[account]={transactions:{},imports:[]}),seen={};let added=0,updated=0;const importId='BST-'+Date.now();parsed.forEach(row=>{const base=[row.date,row.debit,row.credit,row.reference||row.description,row.balance].join('|'),occurrence=seen[base]=(seen[base]||0)+1,key=bankRowKey(account,row,occurrence),existing=book.transactions[key];if(existing){Object.assign(existing,row,{lastSeenImport:importId});updated++;}else{book.transactions[key]=Object.assign({id:'BTX-'+key,firstSeenImport:importId,lastSeenImport:importId},row);added++;}});const dates=parsed.map(x=>x.date).sort(),record={id:importId,file:path.basename(b.filePath||''),originalName:b.originalName||path.basename(b.filePath||''),from:dates[0],to:dates.at(-1),rows:parsed.length,added,updated,uploadedAt:new Date().toISOString(),uploadedBy:b.username||'admin'};book.imports.push(record);audit(s,null,'BANK_STATEMENT_IMPORTED','account',account,{nature,account,user:b.username||'admin',device:b.device||'System',after:record});saveStore(s);return{success:true,account,import:record,transactionCount:Object.keys(book.transactions).length};}
async function importBankStatementUpload(input){const b=input||{},parsed=await parseBankStatementUpload(b.filePath,b.originalName);return storeBankStatementImport(parsed,b);}
router.post('/api/expenses/bank-statements/import',(req,res,next)=>isAdmin(req)?next():res.status(403).json({success:false,error:'Owner/Admin only.'}),statementUpload.single('statement'),async(req,res)=>{if(!req.file)return res.status(400).json({success:false,error:'Choose an XLS, XLSX, CSV, PDF or statement image.'});try{const out=await importBankStatementUpload({filePath:req.file.path,originalName:req.file.originalname,account:req.body.account,nature:req.body.nature,username:req.user.username,device:'Web'});return res.status(out.success?200:400).json(out);}catch(e){return res.status(400).json({success:false,error:e.message||'Could not read this bank statement.'});}});
