...
function paytmReportView(s) {
  ...
  const orderMatches=transactions.map(tx=>{
    ...
    const ids=[tx.transactionId,tx.rrn].filter(Boolean).map(String),byId=orders.filter(order=>ids.some(id=>id.length>=6&&new RegExp('(^|\\D)'+id.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')+'(\\D|$)').test(String(order.note||''))));
...
router.get('/api/expenses/account-ledger', (req, res) => {
  ...
  if(nature==='SANKI'){
    ...
    if(account===PAYTM_CLEARING_ACCOUNT){
      ...
      try {const shop=JSON.parse(fs.readFileSync(ORDERS_PATH,'utf8'));Object.values(shop.orders||{}).filter(order=>!order.cancelledAt&&String(order.financialStatus||'').toLowerCase()==='paid').forEach(order=>{const orderNumber=String(order.orderNumber||order.number||order.name||order.id||'').replace(/\D/g,'').replace(/^0+/,''),date=String(order.processedAt||order.createdAt||'').slice(0,10),gateways=(order.paymentGateways||[]).join(' ').toLowerCase();if(!paytmSalesInScope(date)||creditedOrders.has(String(order.id))||['2717','2720'].includes(orderNumber)||(!/paytm/.test(gateways)&&String(order.channel||'').toLowerCase()!=='pos'))return;const gross=roundMoney(num(order.total)-num(order.refundAmount));if(gross<=0)return;entries.push({id:'PAYTM-PENDING/'+order.id,date,kind:'paytm_pending_sale',description:'Shopify sale #'+orderNumber+' · Paytm amount not yet verified',reference:'#'+orderNumber,credit:0,debit:0,orderId:String(order.id),orderNumber,orderTotal:gross});});}catch{/* Shopify orders not synced yet */}
