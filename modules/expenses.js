...
router.get('/api/expenses/balances', (req, res) => {
  ...
  const accounts = ledgerAccountsForNature(s, nature).map(name => {
    ...
    const closingBalance=opening+num(closing.adj[name])+num(closing.collected[name])+num(closing.transferIn[name])-num(closing.transferOut[name])-num(closing.paidOut[name])+num(closing.bankTruthIn[name])-num(closing.bankTruthOut[name]);
    const issues = reconciliationIssues(s, nature, name);
    const exact=isBankLedgerName(name),money=v=>exact?Math.round(num(v)*100)/100:round0(v);return { name, opening: money(opening), topups, received, transferredIn, transferredOut, spent, excludedBankIn:money(excludedBankIn), excludedBankOut:money(excludedBankOut), periodNet:money(topups+received+transferredIn-transferredOut-spent+excludedBankIn-excludedBankOut), balance:money(closingBalance), closingAsOf:to||'today', reconciled:issues.length===0, reconciliationIssues:issues };
  });
...
router.get('/api/expenses/account-ledger', (req, res) => {
  ...
  (s.bankTruthMovements||[]).filter(x=>normalizedNature(x.nature)===nature&&x.account===account).forEach(x=>entries.push({id:x.id,date:x.date,kind:'bank_truth',description:'Bank truth · excluded from business books · '+(x.description||x.reason||'Statement transaction'),reference:x.reference||x.bankTransactionId||x.id,credit:num(x.credit),debit:num(x.debit),note:x.reason||'',by:x.createdBy||''}));
...
function draftReconciliation(s,draft){
  ...
  return{success:true,draftId:draft.id,account:draft.account,statementSummary:draft.summary,summary,rows,linkCandidates,unresolved,currentOpening,proposedOpening,ledgerClosing,bankClosing,balanceDifference,balanceResolved,openingResolution:draft.openingResolution||null,periodRemark:draft.periodRemark||'',canFinalize:unresolved===0&&balanceResolved,canFinalizeTransactions:unresolved===0};
}
