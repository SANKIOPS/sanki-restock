...
function draftReconciliation(s,draft){
  ...
  rows.sort((x,y)=>String((y.bank&&y.bank.date)||(y.app&&y.app.date)||'').localeCompare(String((x.bank&&x.bank.date)||(x.app&&x.app.date)||''))||String(x.id).localeCompare(String(y.id),undefined,{numeric:true}));
...
router.post('/api/expenses/bank-statements/create-incoming',(req,res)=>{
  ...
  if(!isOwner(req)||!canAccessBankDraft(req,s,draft))return res.status(403).json({success:false,error:'Only the Owner can record incoming money for this reconciliation.'});
