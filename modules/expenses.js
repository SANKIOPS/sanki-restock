...
function draftReconciliation(s,draft){
  ...
  const book=(s.bankStatements||{})[bankStatementBookKey(draft.nature,draft.account)]||{},priorQueues=new Map(),priorByBankIndex=new Map(),priorLinkedIds=new Set();Object.values(book.transactions||{}).forEach(x=>{const key=reconciliationBankSignature(x),queue=priorQueues.get(key)||[];queue.push(x);priorQueues.set(key,queue);});(book.imports||[]).forEach(record=>(record.reconciliationRows||[]).forEach(x=>(x.linkedRecordIds||[]).forEach(id=>priorLinkedIds.add(id))));bank.forEach((b,i)=>{const queue=priorQueues.get(reconciliationBankSignature(b));if(queue&&queue.length)priorByBankIndex.set(i,queue.shift());});priorLinkedIds.forEach(id=>{const index=app.findIndex(x=>x.id===id);if(index>=0)used.add(index);});
