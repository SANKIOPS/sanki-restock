...
function summarizeStatementRows(rows,format){
  if(!rows.length)return null;
  const residual=ordered=>ordered.slice(1).reduce((total,row,i)=>total+Math.abs(num(row.balance)-(num(ordered[i].balance)+num(row.credit)-num(row.debit))),0);
  const forward=rows.slice(),reverse=rows.slice().reverse(),ordered=residual(reverse)+.01<residual(forward)?reverse:forward;
  const first=ordered[0],last=ordered.at(-1),opening=Math.round((num(first.balance)-num(first.credit)+num(first.debit))*100)/100,closing=num(last.balance),debits=rows.reduce((n,x)=>n+num(x.debit),0),credits=rows.reduce((n,x)=>n+num(x.credit),0),calculated=Math.round((opening+credits-debits)*100)/100,dates=rows.map(x=>x.date).filter(Boolean).sort();
  return{format,from:dates[0]||'',to:dates.at(-1)||'',openingBalance:opening,closingBalance:closing,totalDebits:Math.round(debits*100)/100,totalCredits:Math.round(credits*100)/100,validated:Math.abs(calculated-closing)<=.01};
}
function parseBankStatementFile(filePath){
  ...
  const out=[];rows.forEach((raw,index)=>{const normalized={};Object.keys(raw).forEach(k=>normalized[String(k).toLowerCase().replace(/[^a-z0-9]/g,'')]=raw[k]);const get=(...keys)=>{for(const k of keys)if(normalized[k]!=null&&normalized[k]!=='')return normalized[k];return '';};
    ...
    const balance=statementNum(get('balance','closingbalance','availablebalance'));if(!date||(!debit&&!credit))return;out.push({date,description,reference,debit:Math.abs(debit),credit:Math.abs(credit),balance,row:index+2});});out.statementSummary=summarizeStatementRows(out,'Excel/CSV');return out;
}
...
function draftReconciliation(s,draft){
  ...
  return{success:true,draftId:draft.id,account:draft.account,nature:normalizedNature(draft.nature),statementSummary:draft.summary,summary,rows,unresolved,currentOpening,proposedOpening,ledgerClosing,bankClosing,balanceDifference,balanceResolved,openingResolution:draft.openingResolution||null,canFinalize:unresolved===0&&balanceResolved};
}
...
router.post('/api/expenses/bank-statements/import',statementUpload.single('statement'),async(req,res)=>{if(!req.file)return res.status(400).json({success:false,error:'Choose an XLS, XLSX, CSV, PDF or statement image.'});const s=loadStore();if(!canAccessBankReconciliation(req,s,req.body&&req.body.nature,req.body&&req.body.account)){try{fs.unlinkSync(req.file.path);}catch{}return res.status(403).json({success:false,error:'You cannot reconcile this bank account.'});}try{const out=await createBankReconciliationDraft({filePath:req.file.path,originalName:req.file.originalname,account:req.body.account,nature:req.body.nature,username:req.user.username,device:'Web'});return res.status(out.success?200:400).json(out);}catch(e){try{if(req.file&&req.file.path)fs.unlinkSync(req.file.path);}catch{}return res.status(400).json({success:false,error:e.message||'Could not read this bank statement.'});}});
...
router.post('/api/expenses/bank-statements/reconcile',(req,res)=>{const b=req.body||{},s=loadStore(),draft=(s.bankReconciliationDrafts||{})[b.draftId];if(!canAccessBankDraft(req,s,draft))return res.status(403).json({success:false,error:'You cannot reconcile this bank account.'});if(b.account&&String(b.account)!==draft.account)return res.status(409).json({success:false,error:'This statement belongs to '+draft.nature+' · '+draft.account+'. Switch back to that account before comparing.'});const out=reconcileBankStatementAccount(draft.account,req.user.username,'Web',b.draftId,draft.nature);res.status(out.success?200:400).json(out);});
router.post('/api/expenses/bank-statements/discard',(req,res)=>{if(!isOwner(req))return res.status(403).json({success:false,error:'Only the Owner can discard an uploaded statement.'});const b=req.body||{},s=loadStore(),draft=(s.bankReconciliationDrafts||{})[b.draftId];if(!draft)return res.status(404).json({success:false,error:'This temporary statement no longer exists.'});if(!canAccessBankDraft(req,s,draft))return res.status(403).json({success:false,error:'You cannot discard this bank statement.'});if(draft.temporaryFile)try{fs.unlinkSync(draft.temporaryFile);}catch{}delete s.bankReconciliationDrafts[b.draftId];audit(s,req,'BANK_RECONCILIATION_DRAFT_DISCARDED','account',draft.account,{nature:draft.nature,account:draft.account,draftId:draft.id,originalName:draft.originalName});saveStore(s);res.json({success:true,draftId:draft.id,account:draft.account,nature:draft.nature});});
