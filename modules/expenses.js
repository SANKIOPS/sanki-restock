...
function draftReconciliation(s,draft){
  ...
  const findMatch=(b,exactOnly)=>{let best=-1,score=-1,amountMatches=false;app.forEach((a,i)=>{if(used.has(i))return;const days=Math.abs((Date.parse(a.date)-Date.parse(b.date))/86400000);if(days>2)return;const sameDirection=(num(a.debit)>0)===(num(b.debit)>0),exactAmount=Math.abs(num(a.debit)-num(b.debit))<=.01&&Math.abs(num(a.credit)-num(b.credit))<=.01;if(!sameDirection||(exactOnly&&!exactAmount))return;let next=exactAmount?(days===0?6:(days===1?5:4)):(days===0?2:0);const bankText=String(b.reference+' '+b.description).toLowerCase(),appText=String(a.id+' '+a.description).toLowerCase();if(b.reference&&appText.includes(String(b.reference).toLowerCase()))next+=4;else{const words=bankText.split(/[^a-z0-9]+/).filter(w=>w.length>4);if(words.some(w=>appText.includes(w)))next+=2;}if(next>score){score=next;best=i;amountMatches=exactAmount;}});return{best,score,amountMatches};};
  ...
  bank.forEach((b,bi)=>{const id='bank-'+bi,resolution=draft.resolutions&&draft.resolutions[id],match=matches.get(bi);if(match){rows.push({id,status:match.amountMatches?(match.score>=6?'matched':'possible_match'):'amount_mismatch',confidence:match.score,bank:b,app:app[match.best],resolution});}else rows.push({id,status:'missing_in_app',bank:b,resolution});});
