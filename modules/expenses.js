...
function draftReconciliation(s,draft){
  ...
  const groupOrder={possible_match:0,amount_mismatch:0,matched:1,resolved:1,already_reconciled:1,missing_in_app:2,missing_in_bank:3};rows.sort((x,y)=>Number(!!y.matchingExclusion)-Number(!!x.matchingExclusion)||(groupOrder[x.status]??9)-(groupOrder[y.status]??9)||String((y.bank&&y.bank.timestamp)||(y.app&&y.app.createdAt)||(y.bank&&y.bank.date)||(y.app&&y.app.date)||'').localeCompare(String((x.bank&&x.bank.timestamp)||(x.app&&x.app.createdAt)||(x.bank&&x.bank.date)||(x.app&&x.app.date)||''))||String(x.id).localeCompare(String(y.id),undefined,{numeric:true}));
