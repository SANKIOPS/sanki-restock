...
async function combineStatementScreenshots(files){
  ...
  const rows=[];for(let index=0;index<files.length;index++){const parsed=await parseBankStatementUpload(files[index].path,files[index].originalname,'');if(!parsed.length)throw new Error('Screenshot '+(index+1)+' did not contain readable transaction rows. Upload a clearer, uncropped image showing the date and amount columns.');parsed.forEach(row=>rows.push(Object.assign({},row,{sourcePage:index+1})));}
  const currentYear=new Date().getFullYear(),badDate=rows.find(row=>{const year=Number(String(row.date||'').slice(0,4));return !year||year<currentYear-2||year>currentYear+1;}),badAmount=rows.find(row=>Math.abs(num(row.debit))>1000000000||Math.abs(num(row.credit))>1000000000||Math.abs(num(row.balance))>10000000000);
  if(badDate||badAmount)throw new Error('The screenshots could not be read reliably: a date or amount was interpreted incorrectly. No preview was created. Upload clearer full-width screenshots, one page per image, or use the bank Excel/CSV export.');
