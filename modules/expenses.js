...
function applyOwnerConfirmedAxis3645Cases(s) {
  ...
  if(!state.completed.kaluAdvance){
    const principal=exactPayment('EX-00032','PAY-001'),later=(s.expenses||{})['EX-00095'];
    if(principal&&Math.abs(num(principal.payment.amount)-200)<.01&&later&&/kalu/i.test(String(later.vendor||''))&&Math.abs(Math.max(0,num(later.amount)-num(later.paidAmount))-160)<.01){
      const advance={id:'VADV-'+String((s.vendorAdvances||[]).length+1).padStart(5,'0'),nature:'SANKI',vendor:later.vendor||principal.expense.vendor||'Kalu Fruits and Flowers',date:'2026-08-24',amount:218,account,remainingAmount:58,applications:[{expenseId:later.id,date:String(later.date||'2026-08-27'),amount:160}],bankReference:'623662965347',note:'₹418 paid: ₹200 expense payment and ₹218 vendor advance; ₹160 later Kalu expense applied against the advance',createdBy:'prashant',createdAt:now,ownerConfirmedMigration:migrationKey};
      s.vendorAdvances.push(advance);later.vendorAdvanceApplications=Array.isArray(later.vendorAdvanceApplications)?later.vendorAdvanceApplications:[];later.vendorAdvanceApplications.push({vendorAdvanceId:advance.id,amount:160,date:later.date,appliedBy:'prashant',appliedAt:now});later.paidAmount=Math.round((num(later.paidAmount)+160)*100)/100;later.status=later.paidAmount>=num(later.amount)?'paid':(later.paidAmount>0?'partially_paid':'approved');
      ...
      state.completed.kaluAdvance=true;state.results.kaluAdvance={principalId:principal.id,vendorAdvanceId:advance.id,originalAdvance:218,appliedTo:later.id,appliedAmount:160,remainingAmount:58};changed=true;
...
function reconciliationIssues(s, nature, account) {
  ...
  (s.transfers || []).filter(x => {
    ...
    if (!x.proof&&!x.bankStatementEvidence) issues.push({ code:'missing_transfer_proof', reference:x.id || '(missing)', message:'Transfer proof is missing.' });
  });
  return issues;
}
...
router.get('/api/expenses/account-ledger', (req, res) => {
  ...
  (s.vendorAdvances||[]).filter(x=>normalizedNature(x.nature)===nature&&x.account===account).forEach(x=>entries.push({id:x.id,date:x.date,kind:'vendor_advance',description:'Vendor advance · '+x.vendor+' · '+x.note,credit:0,debit:num(x.amount),proof:x.proof||'',reference:x.bankReference||x.id,by:x.createdBy||''}));
...
function appBankMovements(s,account,nature){const rows=[],n=normalizedNature(nature);
  ...
  (s.vendorAdvances||[]).filter(x=>x.account===account&&normalizedNature(x.nature)===n).forEach(x=>rows.push({id:x.id,date:x.date,description:'Vendor advance · '+x.vendor,debit:num(x.amount),credit:0}));
...
router.post('/api/expenses/bank-statements/resolve',(req,res)=>{
  ...
  const view=draftReconciliation(s,draft),row=view.rows.find(x=>x.id===b.rowId),allowed=['accept_match','link_existing','create_adjustment','create_split_adjustment','create_internal_transfer','paytm_settlement','split_allocation','opening_vendor_payable_split','timing_difference','exclude'];
  ...
  if(['timing_difference','exclude','create_adjustment','create_split_adjustment','create_internal_transfer','paytm_settlement','split_allocation','opening_vendor_payable_split'].includes(b.action)&&!String(b.reason||'').trim())return res.status(400).json({success:false,error:'Enter a reason for this resolution.'});
  ...
  draft.resolutions=draft.resolutions||{};draft.resolutions[b.rowId]={action:b.action,reason:String(b.reason||'').trim(),category,chargeCategory,appId:['opening_vendor_payable_split','link_existing'].includes(b.action)?String(b.appId||'').trim():row.app&&row.app.id,otherAccount:String(b.otherAccount||'').trim(),grossAmount:num(b.grossAmount),chargeAmount:num(b.chargeAmount),paytmChargeAmount:num(b.paytmChargeAmount),hiddenChargeAmount:num(b.hiddenChargeAmount),orderIds:Array.isArray(b.orderIds)?b.orderIds.map(String).filter(Boolean):[],transferIds:Array.isArray(b.transferIds)?b.transferIds.map(String).filter(Boolean):[],otherReceipts:Array.isArray(b.otherReceipts)?b.otherReceipts:[],principalAmount:num(b.principalAmount),openingPayableAmount:num(b.openingPayableAmount),openingNature:normalizedNature(b.openingNature||draft.nature),vendor:String(b.vendor||'').trim(),preSystemDates:String(b.preSystemDates||'').trim(),linkedRowId:b.action==='opening_vendor_payable_split'?(view.rows.find(x=>x.app&&x.app.id===String(b.appId||'').trim())||{}).id||'':String(b.linkedRowId||''),remark:String(b.remark||''),linkedTransferAmount:(Array.isArray(b.transferIds)?b.transferIds:[]).reduce((n,id)=>{const t=(s.transfers||[]).find(x=>x.id===id);return n+num(t&&t.amount);},0),by:req.user.username,at:new Date().toISOString()};
