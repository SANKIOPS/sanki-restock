...
async function combineStatementScreenshots(files){
  ...
  const images=[];for(const file of files)images.push(await Jimp.read(file.path));
  const width=Math.max(...images.map(image=>image.bitmap.width)),height=images.reduce((sum,image)=>sum+image.bitmap.height,0),combined=new Jimp(width,height,0xffffffff);let top=0;
  images.forEach(image=>{combined.composite(image,0,top);top+=image.bitmap.height;});
  const filePath=path.join(STATEMENT_DRAFT_DIR,Date.now()+'-'+crypto.randomBytes(4).toString('hex')+'.png');await combined.writeAsync(filePath);
  files.forEach(file=>{try{fs.unlinkSync(file.path);}catch{}});return{filePath,originalName:'Statement screenshots ('+files.length+').png'};
}
