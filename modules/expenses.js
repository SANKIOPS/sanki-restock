...
function draftReconciliation(s,draft){
  ...
  bank.forEach((b,bi)=>{let best=-1,score=-1,amountMatches=false;app.forEach((a,i)=>{if(used.has(i))return;const days=Math.abs((Date.parse(a.date)-Date.parse(b.date))/86400000);if(days>2)return;const sameDirection=(num(a.debit)>0)===(num(b.debit)>0),bankAmount=num(b.debit)+num(b.credit),appAmount=num(a.debit)+num(a.credit),amountDifference=Math.abs(bankAmount-appAmount),exactAmount=amountDifference<=.01,smallDifference=amountDifference<=10;if(!sameDirection||(!exactAmount&&!smallDifference))return;const bankText=String(b.reference+' '+b.description).toLowerCase(),appText=String(a.id+' '+a.description).toLowerCase(),referenceMatch=!!b.reference&&appText.includes(String(b.reference).toLowerCase()),words=bankText.split(/[^a-z0-9]+/).filter(w=>w.length>4),textMatch=words.some(w=>appText.includes(w));let next=exactAmount?(days===0?6:(days===1?5:4)):(days===0?2:0);if(referenceMatch)next+=4;else if(textMatch)next+=2;if(next>score){score=next;best=i;amountMatches=exactAmount;}});const id='bank-'+bi,resolution=draft.resolutions&&draft.resolutions[id];if(best>=0&&score>0){used.add(best);rows.push({id,status:amountMatches?(score>=6?'matched':'possible_match'):'amount_mismatch',confidence:score,bank:b,app:app[best],resolution});}else rows.push({id,status:'missing_in_app',bank:b,resolution});});
...
router.post('/api/expenses/bank-statements/resolve',(req,res)=>{
  ...
  draft.resolutions=draft.resolutions||{};draft.resolutions[b.rowId]={action:b.action,reason:String(b.reason||'').trim(),category,chargeCategory,appId:b.action==='opening_vendor_payable_split'?String(b.appId||'').trim():row.app&&row.app.id,grossAmount:num(b.grossAmount),chargeAmount:num(b.chargeAmount),paytmChargeAmount:num(b.paytmChargeAmount),hiddenChargeAmount:num(b.hiddenChargeAmount),orderIds:Array.isArray(b.orderIds)?b.orderIds.map(String).filter(Boolean):[],transferIds:Array.isArray(b.transferIds)?b.transferIds.map(String).filter(Boolean):[],otherReceipts:Array.isArray(b.otherReceipts)?b.otherReceipts:[],principalAmount:num(b.principalAmount),openingPayableAmount:num(b.openingPayableAmount),vendor:String(b.vendor||'').trim(),preSystemDates:String(b.preSystemDates||'').trim(),linkedRowId:b.action==='opening_vendor_payable_split'?(view.rows.find(x=>x.app&&x.app.id===String(b.appId||'').trim())||{}).id||'':String(b.linkedRowId||''),remark:String(b.remark||''),linkedTransferAmount:(Array.isArray(b.transferIds)?b.transferIds:[]).reduce((n,id)=>{const t=(s.transfers||[]).find(x=>x.id===id);return n+num(t&&t.amount);},0),by:req.user.username,at:new Date().toISOString()};
...
function applyFinalizedOpeningVendorPayables(draft,username){
  ...
  Object.entries(draft.resolutions||{}).forEach(([rowId,r])=>{
    ...
    audit(s,null,'OPENING_VENDOR_PAYABLE_PAID','vendor',vendor,{user:username,device:'Web',nature:draft.nature,account:draft.account,after:payable,note:r.reason,draftId:draft.id});
  });
  saveStore(s);
}
