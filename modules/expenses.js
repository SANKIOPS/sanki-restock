...
function parseBankStatementText(raw){
  ...
  if(/Detailed\s*Statement/i.test(text)&&/ICICI\s*BANK/i.test(text)&&/Withdra\s*wal\s*\(Dr\)/i.test(text)){
    ...
    const debits=out.reduce((n,x)=>n+x.debit,0),credits=out.reduce((n,x)=>n+x.credit,0),calculated=Math.round((opening+credits-debits)*100)/100,valid=out.length&&Math.abs(calculated-closing)<=0.01&&Math.abs(debits-withdrawals)<=0.01&&Math.abs(credits-deposits)<=0.01;if(out.length&&!valid)throw new Error('ICICI statement validation failed: parsed totals do not match the Page Total. No preview was created.');out.statementSummary={format:'ICICI Bank PDF',from:out[0]&&out[0].date,to:out.at(-1)&&out.at(-1).date,openingBalance:opening,closingBalance:closing,totalDebits:Math.round(debits*100)/100,totalCredits:Math.round(credits*100)/100,validated:!!valid};if(out.length)return out;
  }
...
function draftReconciliation(s,draft){
  ...
  bank.forEach((b,bi)=>{let best=-1,score=-1,amountMatches=false;app.forEach((a,i)=>{if(used.has(i))return;const days=Math.abs((Date.parse(a.date)-Date.parse(b.date))/86400000);if(days>2)return;const sameDirection=(num(a.debit)>0)===(num(b.debit)>0),exactAmount=Math.abs(num(a.debit)-num(b.debit))<=.01&&Math.abs(num(a.credit)-num(b.credit))<=.01;if(!sameDirection)return;let next=exactAmount?(days===0?6:(days===1?5:4)):(days===0?2:0);const bankText=String(b.reference+' '+b.description).toLowerCase(),appText=String(a.id+' '+a.description).toLowerCase();if(b.reference&&appText.includes(String(b.reference).toLowerCase()))next+=4;else{const words=bankText.split(/[^a-z0-9]+/).filter(w=>w.length>4);if(words.some(w=>appText.includes(w)))next+=2;}if(next>score){score=next;best=i;amountMatches=exactAmount;}});const id='bank-'+bi,resolution=draft.resolutions&&draft.resolutions[id];if(best>=0&&score>0){used.add(best);rows.push({id,status:amountMatches?(score>=6?'matched':'possible_match'):'amount_mismatch',confidence:score,bank:b,app:app[best],resolution});}else rows.push({id,status:'missing_in_app',bank:b,resolution});});
...
router.post('/api/expenses/bank-statements/resolve',(req,res)=>{
  ...
  const view=draftReconciliation(s,draft),row=view.rows.find(x=>x.id===b.rowId),allowed=['accept_match','link_existing','create_adjustment','create_split_adjustment','paytm_settlement','split_allocation','timing_difference','exclude'];
  ...
  if(['timing_difference','exclude','create_adjustment','create_split_adjustment','paytm_settlement','split_allocation'].includes(b.action)&&!String(b.reason||'').trim())return res.status(400).json({success:false,error:'Enter a reason for this resolution.'});
  ...
  draft.resolutions=draft.resolutions||{};draft.resolutions[b.rowId]={action:b.action,reason:String(b.reason||'').trim(),category,chargeCategory,appId:row.app&&row.app.id,grossAmount:num(b.grossAmount),chargeAmount:num(b.chargeAmount),paytmChargeAmount:num(b.paytmChargeAmount),hiddenChargeAmount:num(b.hiddenChargeAmount),orderIds:Array.isArray(b.orderIds)?b.orderIds.map(String).filter(Boolean):[],transferIds:Array.isArray(b.transferIds)?b.transferIds.map(String).filter(Boolean):[],otherReceipts:Array.isArray(b.otherReceipts)?b.otherReceipts:[],principalAmount:num(b.principalAmount),linkedRowId:String(b.linkedRowId||''),remark:String(b.remark||''),linkedTransferAmount:(Array.isArray(b.transferIds)?b.transferIds:[]).reduce((n,id)=>{const t=(s.transfers||[]).find(x=>x.id===id);return n+num(t&&t.amount);},0),by:req.user.username,at:new Date().toISOString()};
