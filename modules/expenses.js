...
router.use('/api/expenses/bank-statements/finalize',(req,res,next)=>{const before=loadStore(),draft=(before.bankReconciliationDrafts||{})[String(req.body&&req.body.draftId||'')];if(!draft)return next();const previousThrough=String(((before.bankStatements||{})[bankStatementBookKey(draft.nature,draft.account)]||{}).reconciledThrough||''),sources=(draft.sourceStatements||[]).map(x=>({originalName:x.originalName||'',fileHash:x.fileHash||'',from:x.from||'',to:x.to||''})),original=res.json.bind(res);res.json=payload=>{if(payload&&payload.success){const after=loadStore(),book=(after.bankStatements||{})[bankStatementBookKey(draft.nature,draft.account)];if(book){if(previousThrough>String(book.reconciledThrough||''))book.reconciledThrough=previousThrough;if(book.lastReconciliation)book.lastReconciliation.through=book.reconciledThrough;const record=(book.imports||[]).at(-1);if(record&&sources.length)record.sourceStatements=sources;saveStore(after);payload.reconciledThrough=book.reconciledThrough;}}return original(payload);};next();});
