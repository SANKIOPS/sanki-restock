...
function load() {
  try {
    ...
    const guardAugustIncrement=entity==='SANKI'&&applyGuardAugustIncrement(s);
    if(julyImported||employeeRepair||correctedJuly||normalizedLeaveMarks||finalJulyPayroll||correctedAshpreetAdvance||allocatedAshpreetSalary||removedHistoricalAdvances||closedHistoricalPayroll||reopenedAugustPayroll||finalAugustRosterRepair||finalAugustSourceLinks||guardAugustIncrement) save(s);
    return s;
  } catch { return blank(); }
}
...
function computeMonth(s, ym) {
  return Object.values(s.employees).filter(e=>employeeInPayrollMonth(e,ym)&&!((s.months[ym]&&s.months[ym].rows||{})[e.id]||{}).sheetExcluded).sort(byEmployeeName).map(e => {
...
router.post('/api/salary/advances', guard, (req, res) => {
  ...
  if(duplicate)return res.status(409).json({success:false,error:'A matching advance already exists for this employee. Check the employee advance register before recording another.'});
  if (isOwner(req)) {
    const proofs=Array.from(new Set([].concat(Array.isArray(b.proofs)?b.proofs:[],b.proof||[]).map(x=>String(x||'').trim()).filter(Boolean)));
  ...
  const requestProofs=Array.from(new Set([].concat(Array.isArray(b.proofs)?b.proofs:[],b.proof||[]).map(x=>String(x||'').trim()).filter(Boolean)));
  s.advanceRequests[id]={id,empId:emp.id,employeeName:emp.name,amount:round2(amount),date:String(b.date),account:String(b.account).trim(),note:String(b.note||'').trim(),reference:String(b.reference||'').trim(),proof:requestProofs[0]||'',proofs:requestProofs,recoveryStartMonth,status:'Pending approval',createdBy:req.user&&req.user.username||'admin',createdAt:now};
...
router.post('/api/salary/advance-requests/:id/post',guard,(req,res)=>{
  ...
  const s=load(),r=(s.advanceRequests||{})[req.params.id],b=req.body||{},payoutDate=String(b.payoutDate||'').trim(),requestProofs=r&&payoutDate===r.date?r.proofs||[r.proof]:[],proofs=Array.from(new Set([].concat(Array.isArray(b.proofs)?b.proofs:[],b.proof||[],requestProofs||[]).map(x=>String(x||'').trim()).filter(Boolean)));
  ...
  if(!/^\d{4}-\d{2}-\d{2}$/.test(payoutDate)||isNaN(Date.parse(payoutDate+'T00:00:00Z')))return res.status(400).json({success:false,error:'Actual payout date is required before posting.'});
...
function finalAugustPlan(s,file){
  ...
  const seen=new Set(),salaryRows=rows.map((source,index)=>{
    ...
    const difference=round2(actualBank-bank),roundingDifference=Math.abs(difference)<=.5?difference:0,additionalPaid=round2(difference-roundingDifference),laterBank=actual.filter(p=>p.account==='Prashant Axis 3645'&&p.date>'2026-09-12'),laterBankTotal=round2(laterBank.reduce((n,p)=>n+num(p.amount),0));
    if(Math.abs(actualCash-cash)>.005||difference<-.5||actual.some(p=>!['Gagan Sir Cash','Prashant Axis 3645'].includes(p.account))||(additionalPaid>.005&&(Math.abs(laterBankTotal-additionalPaid)>.5||additionalPaid>Math.max(0,round2(net-cash-bank))+.5)))throw new Error(emp.name+' paid amounts differ from the existing cash/3645 payments. Do not duplicate or rewrite payments.');
    return {empId:emp.id,name:emp.name,monthlySalary,paidDays,earned,adjustment,advanceDeduction,net,cash,bank,actualCash,actualBank,roundingDifference,additionalPaid,attendance:daily,sourceRow:index+2};
  });
  ...
  return {hash,stateHash,salaryRows,advances,extras,unmatchedAdvances,totals:{cash:171000,bank:102703,supplemental,actualBank:sum(salaryRows,'actualBank'),actualPaid:round2(sum(salaryRows,'actualCash')+sum(salaryRows,'actualBank')),advance:81850,net:sum(salaryRows,'net')},previewToken:crypto.createHash('sha256').update(hash+stateHash).digest('hex')};
}
function applyFinalAugustPlan(s,plan,by,fileName){
  ...
  mo.finalized=false;
  const actual=computeMonth(s,ym),mismatch=plan.salaryRows.find(x=>{const r=actual.find(y=>y.id===x.empId);return !r||Math.abs(r.netPayable-x.net)>.015||Math.abs(r.transactionPaid-(x.actualCash+x.actualBank))>.005;});
...
function applyGuardAugustIncrement(s){
  const key='guard_august_2026_increment_after_final_workbook_v1',ym='2026-08';
  s.oneTimeMigrations=s.oneTimeMigrations||{};
  if(s.oneTimeMigrations[key]||!(s.finalAugustSheetAudit||[]).length)return false;
  const guards=Object.values(s.employees||{}).filter(e=>String(e.name||'').trim().toLowerCase()==='guard');
  if(guards.length!==1)return false;
  const guard=guards[0],increment=salaryHistoryOf(guard).find(x=>x.effectiveMonth===ym&&num(x.updatedSalary)===16000);
  const row=((s.months||{})[ym]||{}).rows?.[guard.id];
  if(!increment||!row||num(row.sheetMonthlySalaryOverride)!==15000)return false;
  const before=JSON.parse(JSON.stringify(row));
  row.sheetMonthlySalaryOverride=16000;
  row.sheetPaidDaysReason='Final August workbook attendance; Guard increment effective August 2026';
  s.oneTimeMigrations[key]={appliedAt:new Date().toISOString(),empId:guard.id,incrementId:increment.id,before,after:{sheetMonthlySalaryOverride:16000},rule:'Apply confirmed August Guard increment to salary earned; preserve the recorded ₹15,000 payment and leave ₹1,000 payable.'};
  const posting=(s.payrollPostings||{})[ym];
  if(posting){const calculated=computeMonth(s,ym).find(x=>x.id===guard.id),posted=(posting.rows||[]).find(x=>x.empId===guard.id);if(posted&&calculated){posted.salaryAmt=calculated.salaryAmt;posted.netPayable=calculated.netPayable;posting.correctedAt=s.oneTimeMigrations[key].appliedAt;posting.correctedBy='Owner-confirmed Guard increment';}}
  return true;
}
