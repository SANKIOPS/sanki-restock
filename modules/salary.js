...
function applyCorrectedJulyAttendanceV7(s){
  const key='corrected_july_attendance_fixed_leave_allowance_v8';s.oneTimeMigrations=s.oneTimeMigrations||{};
  ...
  JULY_2026_IMPORT.forEach(([name,post,marks,,joiningDate])=>{
    const emp=findImportedEmployee(s,name,post);if(!emp)return;if(joiningDate)emp.joiningDate=joiningDate;emp.monthlyPaidLeaveAllowance=String(name).toLowerCase()==='suraj'?1:4;
