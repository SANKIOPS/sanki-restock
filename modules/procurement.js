...
const AI_IMAGE_SPECS = [
  ...
  { type: 'front',  label: 'Product front', aspect: '4:5',
    prompt: 'Create a BRAND-NEW professional e-commerce FLAT-LAY packshot of this garment. This is NOT a copy task — do NOT return, crop, filter, recolour or re-frame the reference photo; re-photograph the garment completely from scratch as a fresh studio image. Keep only the garment\'s DESIGN faithful — the same colour, print, graphics, text, cut, length and proportions — but RE-SHOOT it: lay the FRONT of the garment flat and face-up, neatly arranged, smooth and wrinkle-free, sleeves relaxed at the sides, so it holds a clean, well-shaped silhouette like a premium catalogue product. CRITICAL: there is NO body inside the garment. Do NOT render any mannequin, dummy, tailor\'s form, dress form, bust, torso, neck, head, shoulders, hands, arms, legs or any human or body-shaped object — not even a hidden, invisible, white or skin-toned one. The neckline and collar opening must simply show the pure-white background behind, NEVER a mannequin neck or a filled-in form. It is JUST the empty garment lying flat on the surface. Completely REMOVE everything in the reference that is not this single garment — any background, table, floor, hanger, packaging, plastic wrap, paper, tags, stickers, price labels, pins, clips, accessories, other clothes or objects nearby must NOT appear. Do NOT copy stray marks, folds, shadows or clutter from the reference. Center the garment on a seamless pure-white studio background, even soft lighting from above, sharp product detail, no human model, no props, no hanger, no text or watermark. Do NOT show any inner neck label, brand tag, size tag or care label — the collar/neckline must be clean. Show a SINGLE garment fully in frame — no duplicated or repeated copies, no collage, no second view. IGNORE the reference photo\'s own edges, corners, border, vignette and background colour; do NOT reproduce any coloured or dark rectangle, rounded corner, drop shadow or padding around it. The pure-white background must fill the ENTIRE frame and bleed to ALL FOUR CORNERS — absolutely no black bars, letterboxing, borders, rounded edges, vignette or leftover colour carried over from the source photo.' },
