...
const AI_IMAGE_SPECS = [
  ...
  { type: 'front',  label: 'Product front',
    prompt: 'Generate a clean product-only photo showing ONLY the FRONT of this exact garment, reproduced FAITHFULLY from the reference image — identical colour, print, graphics, text, cut, length, shape and proportions; do NOT redesign, restyle, reshape or invent details. CRITICAL: reproduce ONLY the garment itself and NOTHING ELSE from the reference. Completely IGNORE and REMOVE anything in the reference that is not this single garment — any background, table, floor, surface, hanger, mannequin, hands, packaging, plastic wrap, paper, tags, stickers, price labels, buttons, pins, clips, accessories, other clothes or objects lying nearby must NOT appear in the result. Do NOT copy stray marks, folds, shadows, clutter or extra items from the reference photo. If the reference shows the garment folded, creased, wrinkled, held, hanging or lying on a surface, instead present it as a smooth, flat, wrinkle-free front-facing catalogue product shot. Center it on a pure white background so the whole front is clearly visible. Even studio lighting, no model, no props, no hanger, no text or watermark, sharp product detail. Do NOT show any inner neck label, brand tag, size tag or care label — the collar/neckline must be clean with no visible tag. Show a SINGLE garment fully in frame; no duplicated or repeated copies, no collage, no second view. Treat the reference ONLY as a guide to the garment\'s shape, colour and print — do NOT paste, embed, screenshot, frame or reuse the reference image itself. IGNORE the reference photo\'s own edges, corners, frame, outline, border, vignette and its background colour; do NOT reproduce any coloured or dark rectangle, rounded corner, drop shadow or padding that surrounds the reference. Redraw the garment fresh, floating on a seamless pure-white studio backdrop. The background must be pure white filling the ENTIRE frame and bleeding to ALL FOUR CORNERS — every corner must be the same pure white with absolutely no black bars, letterboxing, borders, rounded edges, vignette, edge artefact or leftover colour carried over from the source photo.' },
